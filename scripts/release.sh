...
if [[ $branch != "staging" ]];
then
  echo "You must be on the staging branch to release"
  exit
fi;

git pull --ff-only
echo "Running lerna version minor..."
lerna version prerelease --allow-branch $(git branch --show-current) --preid $(git branch --show-current) --no-private --yes
