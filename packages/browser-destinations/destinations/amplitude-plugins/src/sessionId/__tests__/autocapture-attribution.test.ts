...
describe('ajs-integration', () => {
  describe('New session, with no attribution values in the URL', () => {
   
    const example: Subscription[] = [
      {
        partnerAction: 'sessionId',
        name: 'Session and Autocapture Attribution Plugin',
        enabled: true,
        subscribe: 'type = "track"',
        mapping: {
          enableAutocaptureAttribution: true
        }
      }
    ]
    ...
    test('updates the original event with with attributions values from the URL, caches the values, then updates when new values come along', async () => {
      ...
      /* 
      *  Event will include empty attribution values since this is a new session and there are no attribution values in the URL
      */
