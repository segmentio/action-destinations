import {
  ...
  FBClient,
  FBStandardEventType,
  FBNonStandardEventType,
  FBClientParamBuilder,
  PIIType,
  PIIParamName
} from '../types'
...
async function maybeSendUserData(
  client: FBClient,
  clientParamBuilder: FBClientParamBuilder | undefined,
  payload: Payload,
  settings: Settings,
...
export async function formatUserData(
  ...
  const { external_id, em, ph, fn, ln, ge, db, ct, st, zp, country, fbp, fbc } = userData
  ...
  const [emData, phData, fnData, lnData, geData, dbData, ctData, stData, zpData, countryData, externalIdData] =
    await Promise.all([
