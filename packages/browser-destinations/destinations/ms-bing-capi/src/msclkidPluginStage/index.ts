...
const action: BrowserActionDefinition<Settings, {}, Payload> = {
  title: 'Microsoft Bing msclkid Browser Plugin (Stage)',
  description: 'Enriches all Segment payloads with the Microsoft Bing msclkid Querystring value (stage testing)',
  platform: 'web',
  hidden: false,
  ...
  perform: (_, { context, analytics }) => {
    ...
    if (msclkid) {
      ...
      if (
        context.event.integrations?.All !== false ||
        context.event.integrations['Microsoft Bing CAPI Stage (Actions)']
      ) {
        context.updateEvent(`integrations.Microsoft Bing CAPI Stage (Actions)`, integrationsData)
      }
    }
    return
  }
