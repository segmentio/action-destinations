...
export async function send(request: RequestClient, payload: Payload[]) {
  ...
  const batches = payloads.reduce(
    ...
    {
      addEmail: {
        opPayloads: [] as PayloadWithIndex[],
        operationType: { method: 'POST', type: SCHEMA_TYPES.EMAIL } as OperationType
      },
      addPhone: {
        opPayloads: [] as PayloadWithIndex[],
        operationType: { method: 'POST', type: SCHEMA_TYPES.PHONE } as OperationType
      },
      addMAID: {
        opPayloads: [] as PayloadWithIndex[],
        operationType: { method: 'POST', type: SCHEMA_TYPES.MAID } as OperationType
      },
      removeEmail: {
        opPayloads: [] as PayloadWithIndex[],
        operationType: { method: 'DELETE', type: SCHEMA_TYPES.EMAIL } as OperationType
      },
      removePhone: {
        opPayloads: [] as PayloadWithIndex[],
        operationType: { method: 'DELETE', type: SCHEMA_TYPES.PHONE } as OperationType
      },
      removeMAID: {
        opPayloads: [] as PayloadWithIndex[],
        operationType: { method: 'DELETE', type: SCHEMA_TYPES.MAID } as OperationType
      }
    }
  )
