...
describe('AccoilAnalytics.screen', () => {
  it('should track screen events with default mappings', async () => {
    ...
    expect(responses[0].options.body).toMatch(/"type":\s*"screen"/g)
    expect(responses[0].options.body).toContain('bobby')
    expect(responses[0].options.body).toContain('Home')
  })

  it('should track screen events with custom mappings', async () => {
    nock('https://in.accoil.com').post('/segment').reply(200, {})
