...
const action: ActionDefinition<Settings, Payload> = {
  ...
  performBatch: (request, { payload: payloads, ...rest }) => {
    return sendBatch(
      request,
      payloads.map((payload) => ({ ...rest, action: 'event', payload: mapPayload(payload), type: 'person' }))
    )
  },

  perform: (request, { payload, ...rest }) => {
    return sendSingle(request, { ...rest, action: 'event', payload: mapPayload(payload), type: 'person' })
  }
}
