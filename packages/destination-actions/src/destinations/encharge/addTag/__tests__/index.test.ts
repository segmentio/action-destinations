...
describe('Encharge Tag User', () => {
  it('should tag an user', async () => {
    nock(enchargeRestAPIBase).post('/v1/tags').reply(200, {})
    ...
    const responses = await testDestination.testAction('addTag', {
      event,
  ...
  it('should not delete if no IDs', async () => {
    ...
    await expect(
      testDestination.testAction('addTag', {
        event,
        useDefaultMappings: true,
        settings: {
          apiKey: testAPIKey
