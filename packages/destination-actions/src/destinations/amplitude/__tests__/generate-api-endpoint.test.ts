import generateApiEndpoint from '../generate-api-endpoint'

describe('Amplitude - generate API endpoint', () => {
  it('should create a NA endpoint provided and path', () => {
    const path = 'identify'
    const result = generateApiEndpoint(path)
  ...
  it('should create a NA endpoint provided a region and path', () => {
    ...
    const path = 'identify'
    const result = generateApiEndpoint(path, region)
    expect(result).toEqual('https://api.amplitude.com/identify')
  })
  ...
  it('should allow for custom protocol', () => {
    const path = 'identify'
    const result = generateApiEndpoint(path, undefined, { protocol: 'ws' })
  ...
  it('should allow for custom endpoints', () => {
    const path = 'identify'
    const result1 = generateApiEndpoint(path, 'europe', {
      protocol: 'https',
      subdomains: {
        europe: 'api.europe'
      }
    ...
    const result2 = generateApiEndpoint(path, undefined, {
      protocol: 'https',
      subdomains: {
        northAmerica: 'api.us'
      }
    ...
    const result3 = generateApiEndpoint(path, 'europe', {
      protocol: 'https',
      subdomains: {
        northAmerica: 'api.us'
      }
    ...
    const result4 = generateApiEndpoint(path, 'europe', {
      protocol: 'https',
      subdomains: {
        northAmerica: 'api.us',
        europe: 'api.europe'
