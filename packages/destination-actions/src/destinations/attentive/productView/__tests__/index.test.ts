...
const endpoint = '/events/ecommerce/product-view'
...
describe('Attentive.productView', () => {
  it('should send a product view event to Attentive', async () => {
    const event = createTestEvent(validPayload)

    nock(`${API_URL}${API_VERSION}`).post(endpoint, expectedPayload).reply(200, {})

    const responses = await testDestination.testAction('productView', {
      event,
      settings,
  ...
  it('should throw error if no identifiers are provided', async () => {
    ...
    await expect(
      testDestination.testAction('productView', {
        event,
        settings,
        useDefaultMappings: true
      })
