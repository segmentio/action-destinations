...
describe('Send email campaign', () => {
  it('should send campaign with send time optimised option', async () => {
    ...
    nock(settings.api_host).get('/contacts/v3/email/test@example.com').reply(200, { contactId: '123' }) // Correct format for lists

    // Mock send time optimised campaign function
    nock(settings.api_host).post(`/v2/campaigns/send-time-optimised`).reply(200)
  ...
  it('should send campaign without send time optimised option', async () => {
    ...
    // Mock send time optimised campaign function
    nock(settings.api_host).post(`/v2/campaigns/send`).reply(200)
