...
import uploadClickConversion2 from '../index'
...
const fixtures: E2EFixture[] = [
  {
    ...
    // and is accepted. This action is syncMode-gated (default 'add'). order_id is included because,
    // with no gclid/gbraid/wbraid, Google accepts an email-only conversion only as an enhanced
    // conversion for leads, which dedups on order_id.
    description: 'Uploads a single click conversion with basic information',
    subscribe: 'type = "track"',
    mapping: {
      ...defaultValues(uploadClickConversion2.fields),
      conversion_action: CONVERSION_ACTION_ID,
      __segment_internal_sync_mode: 'add'
    },
    mode: 'single',
    event: createE2EEvent('track', 'Order Completed', {
      userId: 'e2e-google-click2-001',
      properties: {
        email: 'e2e-google-click2-001@segment.com',
