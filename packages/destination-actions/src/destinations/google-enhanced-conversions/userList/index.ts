  ...
  hooks: {
    retlOnMappingSave: {
      ...
      inputFields: {
        ...
        partner_link_token: {
          label: 'Data Partner Link Token',
          description:
            'A short-lived OAuth token generated by the advertiser with scope https://www.googleapis.com/auth/datamanager.partnerlink. ' +
            'Provide this once during setup to establish a partner link between your Google Ads account and Segment. ' +
            'After the link is created this token is discarded and never used again. Leave blank if a partner link already exists.',
          type: 'password'
        }
      },
      outputTypes: {
        ...
        partner_link_id: {
          type: 'string',
          label: 'Partner Link ID',
          description:
            'The ID of the Data Manager partner link established between this Google Ads account and Segment.',
          required: false
        }
      },
      performHook: async (request, { auth, settings, hookInputs, features, statsContext }) => {
        ...
        // Step 1 — Partner link creation (one-time, only when token is provided)
        // The advertiser supplies a short-lived token scoped to datamanager.partnerlink.
        // We call Google once to establish the link, then discard the token.
        let partnerLinkId: string | undefined
        if (hookInputs.partner_link_token) {
          try {
            const linkResponse = await createPartnerLink(request, hookInputs.partner_link_token, settings.customerId)
            partnerLinkId = linkResponse.partnerLinkId
          } catch (e) {
            const message = (e as IntegrationError).message || JSON.stringify(e) || 'Failed to create partner link'
            const code = (e as IntegrationError).code || 'CREATE_PARTNER_LINK_FAILURE'
            return { error: { message, code } }
          }
        }

        // Step 2 — Use existing list if list_id provided
