...
import { RequestClient, RetryableError, IntegrationError, InvalidAuthenticationError, Features, FLAGS } from '@segment/actions-core'
...
function getAction(payload: Payload, index: number, audienceMemberships?: AudienceMembership[], features?: Features): 'ADD' | 'REMOVE' {
  ...
  if (dmp_user_action === 'AUTO' || !dmp_user_action) {
    ...
    if (payload.event_name === 'Audience Entered' || membership === true) {
      return 'ADD'
    }

    if (payload.event_name === 'Audience Exited' || membership === false) {
      return 'REMOVE'
    }
  }
