...
describe('Collab Travel CRM', () => {
  ...
  describe('testAuthentication', () => {
    it('should validate authentication with valid API key', async () => {
      ...
      await expect(testDestination.testAuthentication({ apiKey: 'valid-api-key' })).resolves.not.toThrow()
    })
    ...
    it('should fail authentication with invalid API key', async () => {
      ...
      await expect(testDestination.testAuthentication({ apiKey: 'invalid-api-key' })).rejects.toThrow()
    })

    it('should fail authentication when API key is missing', async () => {
      await expect(testDestination.testAuthentication({ apiKey: '' })).rejects.toThrow()
    })
  })
