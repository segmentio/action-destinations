...
export interface Settings {
  ...
  client_id: string
  /**
   * Your Salesforce Marketing Cloud client secret. The client secret is issued when you create an API integration in Installed Packages. Client secrets expire 180 days after they are generated, and separately, as a one-time migration deadline, secrets created before 25 March 2026 expire on 30 September 2026 regardless of when they were generated. When a secret expires Segment can no longer authenticate and event delivery to this destination fails. To rotate without downtime, generate a staged secret in Marketing Cloud Setup > Installed Packages and paste it into this field. Marketing Cloud accepts the staged and the active secret at the same time, so keep the old secret active for at least an hour after saving, then check for deliveries that succeeded in the last few minutes rather than earlier ones from before you saved. Only then activate the new secret in Marketing Cloud. Activating invalidates the old secret immediately, and a longer overlap carries no penalty. See [Rotate an OAuth 2.0 Client Secret](https://developer.salesforce.com/docs/marketing/marketing-cloud/guide/rotate-oauth2-secret.html).
   */
