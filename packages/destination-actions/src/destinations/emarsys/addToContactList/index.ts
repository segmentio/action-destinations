        ...
        if (response && response.status && response.status == 200) {
          if (response.content != '') {
            ...
            if (body.replyCode !== undefined && body.replyCode == 0) {
              ...
              throw new IntegrationError('Something went wront while adding the contact to a contact list')
            }
          } else {
            return response // required to return the empty response for snapshot testing.
          }
        } else if (response.status == 400) {
          throw new IntegrationError('The user could not be added to the contact list')
