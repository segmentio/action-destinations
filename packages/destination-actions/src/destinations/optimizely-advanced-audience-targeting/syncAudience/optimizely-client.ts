...
export class OptimizelyClient {
    ...
    async send() {
        ...
        this.payloads = [this.payloads[0], { ...this.payloads[0], optimizelyUserId: 'user243' }]
        ...
        const requestBody = this.payloads.map((payload) => {
            return {
              ...
              subscription: this.data?.rawData?.properties?.[payload.custom_audience_name] ?? this.data?.rawData?.traits?.[payload.custom_audience_name],
