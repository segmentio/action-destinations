...
class PipedriveClient {
  ...
  async getId(item: ItemType, fieldName: string, term?: string): Promise<number | null> {
    ...
    let result = null
    try {
      const search = await this._request(`${this.settings.domain}/api/v1/itemSearch/field`, {
        searchParams: {
          ...searchParams,
          exact_match: true,
  ...
  async getFields(item: keyof PipedriveFieldTypes): Promise<DynamicFieldResponse> {
    ...
    const url = `${this.settings.domain}/api/v1/${pipedriveFieldMap[item]}`
    console.log('url', url)
    const response = await this._request<PipedriveFields>(url)
  ...
  async getActivityTypes(): Promise<DynamicFieldResponse> {
    const response = await this._request<ActivityTypes>(`${this.settings.domain}/api/v1/activityTypes`)
  ...
  async reqWithPayload(path: string, payload: Record<string, unknown>, method: 'post' | 'put' | 'patch') {
    ...
    const urlBase = `${this.settings.domain}/api/v1`
