...
- A valid `click_id` (Quora "qclid", appended to the landing-page URL after an ad click) is required for a conversion to be attributed; events without one are accepted but not attributed or reported. `user.email` and other identity fields are accepted and stored, but not currently used to attribute conversions.
...
- Email: sent as plaintext; Quora hashes server-side, so the destination does not hash.
...
- Email is mapped and sent as plaintext; Quora hashes server-side, so the destination does not hash.
