...
function buildJSONItem(payload: Payload): RoktJSON {
  ...
  const user_attributes: RoktJSON['user_attributes'] = {
    ...
    ...(billingzipcode
      ? maybeHash(billingzipcode, hashBillingZipcode, 'billingzipcode', 'billingzipsha256', (value) => value.trim())
      : {}),
...
function getAudienceJSON(payload: Payload): AudienceJSON | undefined {
  const {
    ...
    engageFields: { engageAudienceName, traitsOrProps, computationAction } = {}
  } = payload
  ...
  const membership: boolean | undefined =
    isEngageAudience && engageAudienceName
      ? Boolean((traitsOrProps as unknown as Record<string, unknown>)[engageAudienceName])
