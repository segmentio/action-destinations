...
function validate(payload: Payload): string | undefined {
  const {
    device_info: { ios_advertising_id, android_advertising_id, ios_idfv, android_uuid } = {},
    user_identities: { email, customerid } = {},
    rtid
  } = payload
  ...
  if (
    !(
      ...
      trimmed(customerid) ||
      trimmed(rtid) ||
      trimmed(ios_advertising_id) ||
      trimmed(android_advertising_id) ||
      trimmed(ios_idfv) ||
      trimmed(android_uuid)
    )
  ) {
...
function buildJSONItem(payload: Payload): RoktJSON {
  const {
    hashingConfiguration: { hashEmail, hashFirstName, hashLastName, hashMobile, hashBillingZipcode } = {},
    device_info: { http_header_user_agent, ios_advertising_id, android_advertising_id, ios_idfv, android_uuid } = {},
    user_identities: { email, customerid } = {},
    user_attributes: { firstname, lastname, mobile, billingzipcode, dob, gender, ...restUserAttributes } = {},
    ip,
    rtid
  } = payload
  ...
  const user_identities: RoktJSON['user_identities'] = {
    ...(trimmedEmail
      ? maybeHash(trimmedEmail, hashEmail, 'email', 'other', (value) => value.toLocaleLowerCase().trim())
      : {}),
  ...
  const events: RoktJSON['events'] = [...(audienceJSON ? [audienceJSON] : []), ...(eventJSON ? [eventJSON] : [])]
  ...
  const user_attributes: RoktJSON['user_attributes'] = {
    ...(trimmedFirstname
      ? maybeHash(trimmedFirstname, hashFirstName, 'firstname', 'firstnamesha256', (value) => value.trim())
      : {}),
    ...(trimmedLastname
      ? maybeHash(trimmedLastname, hashLastName, 'lastname', 'lastnamesha256', (value) => value.trim())
      : {}),
    ...(trimmedMobile ? maybeHash(trimmedMobile, hashMobile, 'mobile', 'mobilesha256', (value) => value.trim()) : {}),
    ...(trimmedBillingZipcode
      ? maybeHash(trimmedBillingZipcode, hashBillingZipcode, 'billingzipcode', 'billingzipsha256', (value) =>
          value.trim()
        )
      : {}),
    ...(trimmedDob ? { dob: new Date(trimmedDob).toISOString().slice(0, 10).replace(/-/g, '') } : {}),
    ...(gender === 'm' || gender === 'f' ? { gender } : {}),
    ...(restUserAttributes && Object.keys(restUserAttributes).length > 0
      ? sanitize(restUserAttributes, ['boolean', 'string', 'number'], true)
      : {}),
...
function getAudienceJSON(payload: Payload): AudienceJSON | undefined {
  const {
    eventDetails: { source_message_id, timestamp_unixtime_ms },
    audienceDetails: { customAudienceName, customAudienceMembership } = {},
    engageAudienceName,
    traitsOrProps,
    computationAction
  } = payload

  const isEngageAudience =
    engageAudienceName && traitsOrProps && computationAction && typeof traitsOrProps[engageAudienceName] === 'boolean'
...
function getEventJSON(payload: Payload): EventJSON | undefined {
  const {
    eventDetails: { conversiontype, confirmationref, amount, currency, source_message_id, timestamp_unixtime_ms },
    eventProperties
  } = payload
...
function sanitize(
  ...
  Object.entries(obj).forEach(([key, value]) => {
    if (allowedTypes.includes(typeof value as 'string' | 'number' | 'boolean')) {
      ...
      const filtered = (value as unknown[]).filter((v): v is Primitive =>
        allowedTypes.includes(typeof v as 'string' | 'number' | 'boolean')
      )
...
function maybeHash(
  ...
  key: string,
  hashedKey: string,
  cleaningFunction?: (input: string) => string
