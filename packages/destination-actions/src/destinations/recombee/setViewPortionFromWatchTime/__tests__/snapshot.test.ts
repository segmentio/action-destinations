...
describe(`Testing snapshot for ${destinationSlug}'s ${actionSlug} destination action:`, () => {
  ...
  it('should fail when totalLength is negative', async () => {
    const action = destination.actions[actionSlug]
    const [eventData, settingsData] = generateTestData(seedName, destination, action, false)
    eventData.portion.totalLength = new Chance(seedName).floating({ max: -1 })
    eventData.portion.watchTime = new Chance(seedName).floating({ min: 0 })
  ...
  it('should fail when watchTime is negative', async () => {
    const action = destination.actions[actionSlug]
    const [eventData, settingsData] = generateTestData(seedName, destination, action, false)
    eventData.portion.totalLength = new Chance(seedName).floating({ min: 0 })
    eventData.portion.watchTime = new Chance(seedName).floating({ max: -1 })
