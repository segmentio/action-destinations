...
const action: ActionDefinition<Settings, Payload> = {
  title: 'Send Event',
  description: 'Send an event to 1PlusX',
  ...
  fields: {
    ...
    //To be updated with object data type - in the meantime, all properties are sent as top-level k:v pairs below
    custom_fields: {
      label: 'Custom Fields',
  ...
  perform: (request, { settings, payload }) => {
    ...
    const cleanProps = mapValues(custom_fields, (value) => (typeof value === 'string' ? value : JSON.stringify(value)))
