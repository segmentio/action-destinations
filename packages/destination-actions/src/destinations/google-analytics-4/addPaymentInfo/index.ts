...
const action: ActionDefinition<Settings, Payload> = {
  ...
  perform: (request, { payload, features }) => {
    ...
    if (payload.items && !payload.items.length) {
      throw new ValidationError('Google requires one or more products in add_payment_info events.')
    }
    ...
    if (payload.value && payload.currency === undefined) {
      throw new ValidationError('Currency is required if value is set.')
    }
    ...
    if (payload.currency === undefined && payload.items[0].currency === undefined) {
      throw new ValidationError('One of item-level currency or top-level currency is required.')
    }
