...
export async function createSegment(request: RequestClient, region: string, name: string): Promise<string> {
  ...
  const response = await request<CreateSegmentResponse>(`${getDomain(region)}/${SEGMENT_ENDPOINT}/upload`, {
    method: 'POST',
    json
  })
...
export async function getSegment(request: RequestClient, region: string, segmentId: string): Promise<string> {
  const response = await request<GetSegmentResponse>(`${getDomain(region)}/${SEGMENT_ENDPOINT}/${segmentId}`, {
    method: 'GET'
  })
