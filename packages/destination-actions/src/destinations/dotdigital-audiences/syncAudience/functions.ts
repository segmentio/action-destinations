...
export async function send(request: RequestClient, payload: Payload, settings: Settings) {
  ...
  const action: boolean | undefined =
    typeof traits_or_props[computation_key] == 'boolean' ? traits_or_props[computation_key] : undefined

  if (action === undefined) {
    throw new PayloadValidationError(
      `Audience with computation_key = ${computation_key} is missing information about whether to add or remove the contact from the Audience.`
    )
  }
