...
export async function send(
  ...
  const { iterableProjectType } = settings
  const { updateExistingUsersOnly, globalUnsubscribe, campaignId } = audienceSettings || {}
  ...
  payload.map((p, index) => {
    const { email, userId, segmentAudienceKey, traitsOrProperties, dataFields } = p
    ...
    if (iterableProjectType === 'userId' && !userId) {
      ...
      handleError(
        'Either User ID or Email is required when Iterable Project Type = Hybrid',
        isBatch,
        msResponse,
        index,
        p
  ...
  if (unsubscribers.size > 0) {
    const json: UnsubscribePayload = {
      ...
      ...(typeof globalUnsubscribe === 'boolean' ? { channelUnsubscribe: globalUnsubscribe } : {})
    }
    await sendList(
      request,
