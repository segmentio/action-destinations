...
async function readAssociatedRecords(client: Client, groupedPayloads: AssociationPayload[][]): Promise<AssociationPayloadWithId[]> {
...
async function upsertAssociatedRecords(client: Client, groupedPayloads: AssociationPayload[][]): Promise<AssociationPayloadWithId[]> {
    const requests = groupedPayloads.map(async (payloads) => {
        const { object_type: objectType } = payloads[0].object_details
...
function returnAssociatedRecordsWithIds(groupedPayloads: AssociationPayload[][], responses: ModifiedResponse<BatchObjResp>[]): AssociationPayloadWithId[] {
