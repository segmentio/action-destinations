...
export function compareSchemas(schema1: CachableSchema, schema2: CachableSchema | undefined, statsContext?: StatsContext): SchemaDiff {
  ...
  if (schema1.object_details.object_type !== schema2.object_details.object_type) {
    ...
    throw new PayloadValidationError("Hubspot.CustomObject.compareSchemas: Cache error - Schema object_type don't match")
  }
  ...
  if (schema1.object_details.id_field_name !== schema2.object_details.id_field_name) {
    ...
    throw new PayloadValidationError("Hubspot.CustomObject.compareSchemas: Cache error - Schema id_field_name don't match")
  }
  ...
  return {
    match: schema1.properties.length > 0 || schema1.sensitiveProperties.length > 0 ? 'properties_missing' : 'full_match',
...
function compareProps(prop1: Prop, prop2: Prop): boolean {
  if (prop1.name === prop2.name ) {
    if (prop1.type === prop2.type && prop1.fieldType === prop2.fieldType && prop1.typeFieldType === prop2.typeFieldType) {
      return true
    }
    if (prop1.type === prop2.type && prop2.fieldType === 'select' && prop2.type === 'enumeration' && prop1.fieldType === 'text' && prop1.type === 'string') {
