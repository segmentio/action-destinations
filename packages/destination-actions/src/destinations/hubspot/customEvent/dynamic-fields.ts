...
export async function dynamicReadEventNames(request: RequestClient) {
  ...
  try {
    const response: ResponseType = await request(`${HUBSPOT_BASE_URL}/events/v3/event-definitions/?includeProperties=false`, {
      method: 'GET',
...
export async function dynamicReadProperties(request: RequestClient, eventName: string){
  ...
  try {
    ...
    const response: ResponseType = await request(`${HUBSPOT_BASE_URL}/events/v3/event-definitions/?includeProperties=true`, {
      method: 'GET',
      skipResponseCloning: true
    })
