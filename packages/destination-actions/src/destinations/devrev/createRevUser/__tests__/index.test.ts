...
describe('Devrev.createRevUser', () => {
  ...
  it('makes the correct devrev api calls: existing revuser, no comment', async () => {
    nock('https://api.devrev.ai')
      .get('/internal/rev-users.list')
      .query({ email: `"${email}"` })
      .reply(200, revUsersListResponse.data)
  ...
  it('makes the correct devrev api call: existing revuser, with comment', async () => {
    ...
    const response = await testDestination.testAction('createRevUser', {
      settings,
      event,
  ...
  it('makes the correct devrev api calls: no revuser, existing account, no comment', async () => {
    ...
    nock('https://api.devrev.ai')
      ...
      .reply(200, accountsListResponse.data)
    nock('https://api.devrev.ai')
  ...
  it('tags an existing user with a tag', async () => {
    nock('https://api.devrev.ai')
      .get('/internal/rev-users.list')
      .query({ email: `"${email}"` })
      .reply(200, revUsersSingleList.data)
    nock('https://api.devrev.ai').post('/internal/rev-users.update').reply(200, revUserUpdateTagsResponse)
    const mapping = {
      tag: testTag.id
    }
    const response = await testDestination.testAction('createRevUser', {
  ...
  it('makes the correct devrev api calls: no revUser, no account, with tag and comment', async () => {
    nock('https://api.devrev.ai')
      .get('/internal/rev-users.list')
      .query({ email: `"${email}"` })
      .reply(200, { rev_users: [] })
