...
export async function send(
  ...
  const {
    engage_fields: { segment_external_audience_id: audienceId, segment_audience_key: audience_key }
  } = payloads[0]

  const { endpoint } = settings

  const { id_type } = (audienceSettings || {}) as { id_type: IDType | undefined }
  ...
  payloads.forEach((payload, index) => {
    const { engage_fields: { traits_or_properties } = {} } = payload
  ...
  if (requests.length === 0) {
    ...
    throw new PayloadValidationError(
      'The payload is invalid and cannot be sent to Amplitude. Check that it contains the correct type of identifier'
    )
  }
...
export function failAllPayloads(
  ...
  payloads.forEach((payload, index) => {
    handleError(payload, msResponse, index, isBatch, message, 'PAYLOAD_VALIDATION_FAILED')
  })
  if (isBatch) {
    return msResponse
  }
    ...
    if (id) {
      if (idSet.has(id)) {
        ...
        handleError(
          ...
          index,
          isBatch,
          `Duplicate ID ${id} of type ${getIdTypeName(
            id_type
          )} found in payload batch. The duplicate payload has been rejected. Each payload must have a unique ID for the specified ID Type.`,
      ...
      handleError(
        ...
        index,
        isBatch,
        `No User Identifier of type ${getIdTypeName(
          id_type
        )} found in payload. Each payload must have a unique ID for the specified ID Type.`,
        'PAYLOAD_VALIDATION_FAILED'
      )
    }
  }
...
export async function sendRequest(
  ...
  try {
    ...
    const { response: { data: { error: { error, message } = {} } = {} } = {} } = err as ResponseError
    ...
    for (const [index, payload] of map.entries()) {
      ...
      handleError(
        ...
        index,
        isBatch,
        `Request failed for payload with ID ${id} of type ${id_type} with error: ${
          error || 'UNKNOWN_ERROR'
        } and message: ${message || 'No message returned from Amplitude'}`,
