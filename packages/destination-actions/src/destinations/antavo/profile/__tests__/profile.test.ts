...
describe('Antavo (Actions)', () => {
  ...
  describe('profile', () => {
    it('Handle request with default mappings', async () => {
      nock(`https://api.${settings.stack}.antavo.com`).post('/v1/webhook/segment').reply(202, {})
      ...
      const responses = await testDestination.testAction('profile', {
        event,
        settings,
        mapping: {
    ...
    it('Throw error for missing required field: customer', async () => {
      ...
      await expect(
        testDestination.testAction('profile', {
          event,
          settings,
          mapping: {
            data: {
