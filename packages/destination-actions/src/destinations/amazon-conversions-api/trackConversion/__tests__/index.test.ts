...
describe('trackConversion', () => {
  ...
  describe('perform', () => {
    it('should send event data successfully', async () => {
      nock(`${Region.NA}`).post('/events/v1').reply(200, { success: true })
    ...
    it('should handle API errors', async () => {
      ...
      nock(`${Region.NA}`).post('/events/v1').replyWithError('Invalid event data')
  ...
  describe('performBatch', () => {
    ...
    it('should process a batch of events successfully', async () => {
      ...
      nock(`${Region.NA}`)
        ...
        .reply(207, {
          ...
          error: [{ index: 2, httpStatusCode: '400', subErrors: [{ errorMessage: 'Invalid event' }] }]
        })
    ...
    it('should handle API errors in batch mode', async () => {
      nock(`${Region.NA}`).post('/events/v1').reply(500, { error: 'Internal server error' })
