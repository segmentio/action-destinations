...
const defaultTemplate = describe.each(['stage', 'production'])('%s environment', (environment) => {
  const spaceId = 'd'
  const settings = {
    twilioAccountSID: 'a',
    twilioApiKeySID: 'f',
  ...
  const getDefaultMapping = (overrides?: any) => {
    return {
      ...
      externalIds: [{ type: 'ios.push_token', id: 'my-ios-token', subscriptionStatus: 'subscribed' }],
      ...overrides
    }
  }
  ...
  describe.skip('sendPpush notification', () => {
    it('should abort when there is no push notification external ID in the payload', async () => {
      const responses = await twilio.testAction('sendPush', {
    ...
    it('should send notification to every subscribed device', async () => {
      ...
      nock(`https://content.twilio.com`).get(`/v1/Content/${defaultTemplateSid}`).reply(200, defaultTemplate)
      const twilioRequest = nock(`https://content.twilio.com/v1/Content/${defaultTemplateSid}`)
    ...
    it('should fail on invalid webhook url', async () => {
      ...
      await expect(twilio.testAction('sendPush', actionInputData)).rejects.toHaveProperty('code', 'ERR_INVALID_URL')
    })
  })
