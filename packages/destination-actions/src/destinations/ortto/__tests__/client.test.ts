...
describe('Ortto.Client', () => {
  ...
  it('getAudience: should validate empty audience Id', async () => {
    const fakeRequest = jest.fn().mockResolvedValue({
      data: {} // no errors in response
    })

    const client = new OrttoClient(fakeRequest)
    const response = client.getAudience(settings, '')
    await expect(response).rejects.toThrowError(new PayloadValidationError(Errors.MissingAudienceId))
    expect(fakeRequest).not.toHaveBeenCalled()
  })
