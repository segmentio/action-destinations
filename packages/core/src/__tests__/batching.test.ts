...
describe('Async Batching', () => {
  ...
  test('prefers caller-supplied auth over auth derived from settings', async () => {
    const multiStatusResponse = new MultiStatusResponse()
    multiStatusResponse.pushSuccessResponse({ status: 200, body: {}, sent: {} })
    multiStatusResponse.pushSuccessResponse({ status: 200, body: {}, sent: {} })
    ...
    await destination.executeAsyncBatch('asyncTestAction', {
      events,
      mapping: { user_id: { '@path': '$.userId' } },
      settings: { oauth: { access_token: 'from-settings' } },
      auth: { accessToken: 'from-caller', refreshToken: 'refresh-from-caller' }
    })
  ...
  test('falls back to auth derived from settings when the caller omits it', async () => {
    const multiStatusResponse = new MultiStatusResponse()
    multiStatusResponse.pushSuccessResponse({ status: 200, body: {}, sent: {} })
    multiStatusResponse.pushSuccessResponse({ status: 200, body: {}, sent: {} })

    mockPerformBatch.mockResolvedValue({
      jobId: 'auth-fallback-job',
      status: 200,
      multiStatusResponse
    } as AsyncBatchResponse)
    ...
    await destination.executeAsyncBatch('asyncTestAction', {
      ...
      settings: {
        oauth: {
          access_token: 'from-settings',
          refresh_token: 'refresh-from-settings',
    ...
    expect(mockPerformBatch).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        auth: {
          accessToken: 'from-settings',
          refreshToken: 'refresh-from-settings',
...
describe('Async Poll', () => {
  ...
  test('poll prefers caller-supplied auth over auth derived from settings', async () => {
    mockPerformPoll.mockResolvedValue({
      jobId: 'poll-job-auth',
      status: 200,
      jobStatus: 'IN_PROGRESS'
    } as PollResponse)
    ...
    await destination.executeAsyncPoll('asyncPollAction', {
      pollPayload: { jobId: 'poll-job-auth', uploadCount: 1 },
      settings: { oauth: { access_token: 'from-settings' } },
      auth: { accessToken: 'from-caller', refreshToken: 'refresh-from-caller' }
    })
  ...
  test('poll falls back to auth derived from settings when the caller omits it', async () => {
    mockPerformPoll.mockResolvedValue({
      jobId: 'poll-job-auth-fallback',
      status: 200,
      jobStatus: 'IN_PROGRESS'
    } as PollResponse)
    ...
    await destination.executeAsyncPoll('asyncPollAction', {
      pollPayload: { jobId: 'poll-job-auth-fallback', uploadCount: 1 },
      settings: {
        oauth: {
          access_token: 'from-settings',
          refresh_token: 'refresh-from-settings',
