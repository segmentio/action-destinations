...
export class Action<Settings, Payload extends JSONLikeObject, AudienceSettings = any> extends EventEmitter {
  ...
  private setMultiStatusResponse(input: SetMultiStatusResponseInput) {
    ...
    for (let i = 0; i < multiStatusResponse.length(); i++) {
      ...
      results[i] = {
        response: multiStatusResponse.getResponseAtIndex(resultsWriteIndex++)
      }
    }
  }
}
...
export class MultiStatusResponse {
  ...
  public unsafe = {
    ...
    pushSuccessResponse: (response: ActionDestinationSuccessResponse) => {
      this.responses.push(response)
    },

    // Pushes an Error Response at the end of the responses array
    pushErrorResponse: (response: ActionDestinationErrorResponse) => {
      this.responses.push(response)
      this.invalidPayloadIndices.add(this.responses.length - 1)
    },

    // Pushes a Success Response at the specified index of the responses array
    setSuccessResponseAtIndex: (response: ActionDestinationSuccessResponse, index: number) => {
      this.responses[index] = response

      // If the index previously had an invalid payload, remove it from the invalid payload indices
      if (this.invalidPayloadIndices.has(index)) {
        this.invalidPayloadIndices.delete(index)
      }
    },

    // Pushes an Error Response at the specified index of the responses array
    setErrorResponseAtIndex: (response: ActionDestinationErrorResponse, index: number) => {
      this.responses[index] = response
      this.invalidPayloadIndices.add(index)
    }
  }

  public filterInvalidPayloads = <PayloadType extends object, TransformedPayloadType extends object = object>(
    payloads: PayloadType[],
    defaultValidationFailureResponse: ActionDestinationErrorResponse,
    ...
    if (payloads.length !== this.batchSize) {
      throw new IntegrationError(
        `The number of payloads provided (${payloads.length}) does not match the batch size (${this.batchSize}).`,
        'InvalidPayload',
        400
      )
    }
    ...
    const validatedPayloads: object[] = []

    for (let i = 0; i < payloads.length; i++) {
      if (!payloads[i]) {
        this.responses[i] = defaultValidationFailureResponse

        continue
      }

      // Execute the validation function
      const validationResponse = validatorFn(payloads[i])

      // If the validation response is a boolean, we assume it's a simple validation check
      if (typeof validationResponse === 'boolean' && !validationResponse) {
        this.responses[i] = defaultValidationFailureResponse
        continue
      }

      // If the validation response is an object, we assume it's a complex validation check
      if (typeof validationResponse === 'object' && !validationResponse.isValid) {
        if (validationResponse.response) {
          this.responses[i] = validationResponse.response
        } else {
          this.responses[i] = defaultValidationFailureResponse
        }
      }
      ...
      if (transformerFn) {
        const transformedPayload = transformerFn(payloads[i])
  ...
  public mergeAPIResponse = async <APIResponseType = unknown>(
    upstreamResponse: MaybePromise<ModifiedResponse<APIResponseType>>,
    responseParserFn: (
    ...
    const parsedResponses = responseParserFn(response, this.validPayloadIndexToBatchIndexMap.size)
    ...
    if (this.wasFilterInvalidPayloadsExecuted) {
      for (let i = 0; i < parsedResponses.length; i++) {
        this.responses[i] = parsedResponses[i]
      }
    ...
    for (let i = 0; i < parsedResponses.length; i++) {
      ...
      this.responses[resolvedIndex] = parsedResponses[i]
    }
  }

  public getResponseAtIndex(index: number): ActionDestinationResponse | undefined {
