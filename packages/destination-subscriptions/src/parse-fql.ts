  ...
  while (token && token.type !== 'eos') {
    if (token.type === 'ident') {
      ...
      if (conditionType) {
        ...
        if (conditionType === 'event') {
          ...
          } else if (operatorToken.value === '=' && valueToken.value === 'null') {
            nodes.push({
              type: 'event-property',
              name: token.value.replace(/^(properties|traits)\./, ''),
              operator: 'not_exists'
    ...
    if (token.type === 'parenleft') {
      ...
      while (groupToken?.type !== 'parenright') {
        groupTokens.push(groupToken as Token)
...
const normalize = (tokens: Token[]): Token[] => {
  ...
  while (tokens[index]) {
    if (tokens[index].type === 'ident' && tokens[index + 1].type === 'dot' && tokens[index + 2].type === 'ident') {
      normalizedTokens.push({
        type: TokenType.Ident,
        value: tokens[index].value + tokens[index + 1].value + tokens[index + 2].value
      })
