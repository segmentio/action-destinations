...
const validateGroupCondition = (condition: GroupCondition, data: unknown): boolean => {
...
const validate = (condition: Subscription, data: unknown): boolean => {
...
const validateCondition = (condition: Condition, data: any): boolean => {
  ...
  if (condition.type === 'event-property') {
    const properties = ['identify', 'group'].includes(data.type) ? data.traits : data.properties

    return validateValue(get(properties, condition.name), condition.operator, condition.value)
  }
