...
describe('Clevertap.userDelete', () => {
  it('should validate action fields', async () => {
    const event = createTestEvent({event: 'Test Event', userId: 'qwqw'})

    nock(CLEVERTAP_ACCOUNT_ENDPOINT).post('/1/delete/profiles.json').reply(200, {})

    const responses = await testDestination.testAction('userDelete', {
      event,
